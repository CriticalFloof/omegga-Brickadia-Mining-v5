...
import { Block, ChunkData, ChunkVector, MineableBlock, NaturalBlock, Section, SectionVector, SpatialVector, TerrainGeneratorData } from "src/typescript_definitions/plugin";
...
export class SurfaceHandler {
    ...
    public async initializeSurface(name : string, properties?: {
        seed? : number;
     ...
     */
    private addSurface(name : string, properties?: {
        seed? : number;
        block_size? : og.Vector,
        chunk_size? : og.Vector,
        section_size? : og.Vector,
...
export class Surface extends Spatial {
    ...
    public getBlockAtPosition(absolute_position : SpatialVector): Block | NaturalBlock | MineableBlock | void{
        ...
        try {
            ...
            block = OmeggaHandler.game.block_source.blocks[chunk.block_palette[chunk.block_data[BrickLoader.spatial2ArrayFunction(relSpatialPosition, this)]]]       
        } catch (error) {
    ...
    public generateStructure(name: string, absolute_position: SpatialVector):void {
        ...
        const minChunkPosition = this.spatialToChunk([absolute_position[0]-this.chunk_size[0],absolute_position[1]-this.chunk_size[1],absolute_position[2]-this.chunk_size[2]])
        const maxChunkPosition = this.spatialToChunk([absolute_position[0]+bounds[0]+this.chunk_size[0],absolute_position[1]+bounds[1]+this.chunk_size[1],absolute_position[2]+bounds[2]+this.chunk_size[2]])
        ...
        if(structure_data[name].mask !== false){
            ...
            BrickLoader.neighbourUpdateSegment(this, [absolute_position,[absolute_position[0]+bounds[0],absolute_position[1]+bounds[1],absolute_position[2]+bounds[2]]], {quiet: true}).then(()=>{
                Omegga.clearRegion({center : worldMidpoint, extent : worldExtent})
                OmeggaImprovements.loadBricks(`Mine-Test/Structures/${structure_data[name].mask}`, {quiet: true, offX: offset[0], offY: offset[1], offZ: offset[2]})
            })
            
        } else {
            //Load the pure brick representation
            
            BrickLoader.neighbourUpdateSegment(this, [absolute_position,[absolute_position[0]+bounds[0],absolute_position[1]+bounds[1],absolute_position[2]+bounds[2]]], {quiet: true})
        }
    }
    ...
    public chunkToRelative(chunk_position : ChunkVector): ChunkVector {
        ...
        if(relChunkPosition[0] < 0){ relChunkPosition[0] += this.section_size[0]}
        if(relChunkPosition[1] < 0){ relChunkPosition[1] += this.section_size[1]}
        if(relChunkPosition[2] < 0){ relChunkPosition[2] += this.section_size[2]}
    ...
    public spatialToRelative(spatial_position : SpatialVector): SpatialVector {
        ...
        if(relSpatialPosition[0] < 0){ relSpatialPosition[0] += this.chunk_size[0]}
        if(relSpatialPosition[1] < 0){ relSpatialPosition[1] += this.chunk_size[1]}
        if(relSpatialPosition[2] < 0){ relSpatialPosition[2] += this.chunk_size[2]}
     ...
     */
    public findSolidPosition(worldPositions : og.Vector[]): {absolutePosition : og.Vector, memoryPosition:{ relativeSpatial : og.Vector, relativeChunk : og.Vector, section : og.Vector }} | void {
        ...
        for (let i = 0; i < worldPositions.length; i++) {
            ...
            let block: Block = OmeggaHandler.game.block_source.blocks[chunk.block_palette[chunk.block_data[BrickLoader.spatial2ArrayFunction(relSpatialPosition, this)]]]
            ...
            //return position
            return {absolutePosition: spatialPosition, memoryPosition:{ relativeSpatial:relSpatialPosition, relativeChunk:relChunkPosition, section:sectionPosition }}
        }
    }
