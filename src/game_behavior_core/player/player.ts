...
export class Player {
    ...
    public async worldInteract(weapon_name: string){
        ...
        const rotationRegExp = new RegExp(`${this.player_object.controller}\\.TransformComponent0.RelativeRotation = \\(Pitch=(?<x>[\\d\\.-]+),Yaw=(?<y>[\\d\\.-]+),Roll=(?<z>[\\d\\.-]+)\\)`);
        const [{groups: { x, y, z }}] = await Omegga.addWatcher(rotationRegExp, {
            exec: () =>
            Omegga.writeln(
                `GetAll SceneComponent RelativeRotation Outer=${this.player_object.controller}`
            ),
            timeoutDelay: 100
        });
        ...
        if(!tool) return;
        let cameraPosition : og.Vector = isCrouched ? [this.position[0], this.position[1], this.position[2]+11] : [this.position[0], this.position[1], this.position[2]+17]
        let positions = Raycast.spatialDDARaycast(cameraPosition, [parseFloat(x),parseFloat(y),parseFloat(z)], tool.range, OmeggaHandler.game.surfaceHandler.surfaces["Earth"])
    ...
    public levelup(amount: number){
        ...
        if(!creditIndex) { Game.whisper(this.player_object.name, `You cannot afford to level up!`); return;}
        ...
        for(let i = 0; i < realAmount; i++) {
            ...
            if(this.level+1 > (this.rank+1)*100){
                Game.whisper(this.player_object.name, `You have reached your max level and need to <color="ffff00">/rankup</>! For more information about ranking up, use <color="ffff00">/help_mining rankup</>`)
                return
            }
        ...
        if(startingLevel < this.level){
            ...
            Game.whisper(this.player_object.name, `You need ${levelupCost()} credits to upgrade! You currently have ${this.inventory.amount[creditIndex]}`)
        }
    }
    ...
    public rankup(){
        ...
        if(this.level < (this.rank+1)*100){
            Game.whisper(this.player_object.name,`You cannot rank up until you have reached your rank's limit! Rank ${this.rank}'s limit is ${(this.rank+1)*100}.`)
            return
        }
        this.rank += 1
        this.level = 1
    ...
    public sellQuery(requestName: string, amount: number){
        ...
        Game.whisper(this.player_object.name, `You sold ${itemsSold} ${items.length === 1 ? items[0].name : itemsSold === 1 ? 'item' : 'items'} for ${amountGained} ${amountGained === 1 ? 'credit':'credits'}!`)
    ...
    public addItem(ItemName: string, amount: number = 1): number{
        ...
        if(this.inventory.amount[foundIDIndex] <= 0){
            ...
            this.inventory.amount = this.inventory.amount.filter((val) => {return val !== null});
            this.inventory.itemID[foundIDIndex] = null
            this.inventory.itemID = this.inventory.itemID.filter((val) => {return val !== null});
    ...
    public openInventory(selectedPage: number){
        ...
        setTimeout(()=>{this.context_state = 'world'},60*60000)
    ...
    public sendNotification(message: string = '', options?:{timeout?: number, duration?: number, type?: 'lazy' | 'alert' | 'context_overwrite' | 'overwrite', group?: string}){
        if(!options) options = {};
    ...
    private nextInUiQueue() {
        ...
        const id = UiHandler.draftRequest(this.ui_queue[0].timeout, [this.player_object.name], 'middle_print', this.ui_queue[0].duration, this.ui_queue[0].message)
