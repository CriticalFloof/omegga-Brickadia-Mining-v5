...
export class LoadManager {
    ...
    public static async loadPlayer(name: string){
        let filePath = SaveManager.savePath; if(!filePath) return;
        filePath += `/Players/${name}`
        let fileContents
        try {
            fileContents = await fsp.readFile(`${filePath}.txt`, {encoding:"utf-8"})
        } catch (error) {
            return
        }
    ...
    public static async loadSurface(name: string, properties?: {
        seed? : number;
    ...
    private static async loadSection(filePath: string): Promise<Section>{
        ...
        let fileContents = await fsp.readFile(`${filePath}.txt`, {encoding:"utf-8"})
        ...
        function buildLayer(stringsToSearch: string[] | RegExpMatchArray,) {
            ...
            fileContents = fileContents.replace(/{[^{}]*}/g, () => {currentID++; return `<${currentID-1}>`})
